...
```{r estriol, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap="Correlação da excreção de estriol urinário e peso fetal", fig.pos="ht"}
...
```{r pesorenda, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap="Gráfico de dispersão sugerindo ausência de correlação.", fig.pos="ht"}
...
Ass medidas resumidoras serão calculadas, usando a função `get_summary_stats ()` do pacote `rstatix` que necessita dos seguintes argumentos:
...
A separação dos pontos por sexo não muda a análise e foi realizada apenas para treinamento (e curiosidade!).
...
Vamos discutir sete pressupostos, três estão relacionados com o projeto do estudo e como as variáveis foram medidas (pressupostos 1, 2 e 3) e quatro que se relacionam com as características dos  dados (pressupostos 4, 5, 6 e 7) [@kassambara2021correlation].
...
Para verificar se as variáveis têm distribuição normal, é possível usar o teste de Shapiro-Wilk, usando a função `shapiro.test()`, incluída no *R* base:
...
O teste estatístico fornece uma prova, mas também pode-se fazer uma verificação visual rápida da normalidade dos dados, usando a função `ggqqplot()` do pacote `ggpubr`que produz um gráfico quantil-quantil, onde os pontos devem cair próximos da linha de referência (Figuras \@ref(fig:qqidade) e \@ref(fig:qqcomp).

```{r qqidade, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Gráfico QQ da idade."}
...
```{r qqcomp, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Gráfico QQ do comprimento"}
ggpubr::ggqqplot(dados,
                 x = "comp",
                 color = "steelblue",
                 xlab = "Quantis normais",
...
A presença de *outliers* pode ser verificada construindo boxplots (Figura \@ref(fig:atipicos)) com as variáveis `idade` e `comp`

```{r atipicos, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Pesquisa de valores atípicos com boxplots"}
par (mfrow=c (1,2))
boxplot(dados$idade, 
       ylab = "Idade (meses)",
       col = "lightgreen") 
...
Para ajustar a um modelo linear, usa-se a função `lm()` que deve conter um objeto da classe `formula` como argumento. Demais características da função podem ser obtidas com `?lm` ou direto na ajuda do `RStudio`. O modelo será atribuído a um objeto denominado `mod_reg`.

```{r}
mod_reg <- lm(comp ~ idade, dados)
```

*Análise gráfica da homoscedasticidade* (Figura \@ref(fig:homo1)):
...
Os gráficos de diagnóstico dos resíduos são apresentados de quatro maneiras diferentes e, para maioress detalhes consulte [**aqui**](https://data.library.virginia.edu/diagnostic-plots/) .O  primeiro e terceiro tipo serão avaliados:

**Gráfico 1**: *valores previstos $\times$ resíduos*  
...
**Gráfico 3**: *valores previstos $\times$ resíduos padronizados*  
...
Embora o gráfico possa dar uma ideia sobre homocedasticidade, às vezes, um teste mais formal é preferido. Existem vários testes para isso, mas aqui será utilizado o *Teste de Breusch-Pagan*. A $H_{0}$ e a $H_{A}$ podem ser consideradas como:

$H_{0}$: Homocedasticidade. Os resíduos têm variância constante sobre o modelo verdadeiro.  

$H_{A}$: Heterocedasticidade. Os resíduos não têm variância constante sobre o modelo verdadeiro.
...
* **x**           $\longrightarrow$ dados da variável *x*;
* **y**           $\longrightarrow$ dados da variável *y*; 
* **method**      $\longrightarrow$ Pode ser usado dos seguintes "pearson", "kendall" ou "spearman”; exact | indicação lógica se um valor *P* exato deve ser calculado. Usado para o $\tau$ de Kendall e  $\rho$ de Spearman;
* **alternative** $\longrightarrow$  hipótese alternativa “two.sided” (bilateral) ou “greater” ou “less” (unilateral a direita ou a esquerda, respectivamente);
conf.level | nivel de confiança. Padrão 0.95.
...
A saída do Coeficiente de Correlação de Pearson (*r*) é igual  `r round(r$estimate, 2)` (IC95%: `r round (r$conf.int, 2)`) o que corresponde a uma correlação linear muito forte (Tabela \@ref(tab:coefr), ignorando o sinal) entre a idade e o comprimento de crianças [@schober2018correlation].    

O coeficiente refere a existência de correlação linear, mas não especifica se a relação é de causa e efeito. O valor *P* especifica se a correlação é igual a zero ($H_{0}$) ou diferente de zero ($H_{A}$). No caso, ela é diferente de zero.  
...
Os gráficos de diagnóstico da regressão (Figura \@ref(fig:diagreg)) podem ser criados usando a função `plot()` do R base, como mostrado para a correlação:
...
Além de observar o gráfico QQ plot, é possível realizar um teste estatístico de normalidade dos resíduos como, por exemplo o teste de Shapiro-Wilk. Ele pode ser executado, usando a função shapiro.test() do pacote stats, incluído no R base.  
...
A variância constante, homogênea, é frequentemente chamada de homocedasticidade. Por outro lado, a variância não constante é chamada de heterocedasticidade. Foi visto que é possível usar um gráfico de `resíduos versus ajustados` (previstos) e o `scale-location` (gráficos diagnósticos) para avaliar esse pressuposto. Embora o gráfico possa dar uma ideia sobre homocedasticidade, às vezes, um teste mais formal é preferido. Existem vários testes para isso, mas aqui será utilizado o `Teste de Breusch-Pagan`. A $H_{0}$ e a $H_{A}$ podem ser consideradas como,

$H_{0}$: *Homocedasticidade*. Os resíduos têm variação constante sobre o modelo verdadeiro.

$H_{A}$: *Heterocedasticidade*. Os resíduos não têm variância constante sobre o modelo verdadeiro.

Se o valor *P* > 0,05 não se rejeita a $H_{0}$ de homocedasticidade. O teste de Breusch-Pagan é encontrado na função `bptes()`, incluída no pacote `lmtest`[@hothorn2015lmtest]. 
