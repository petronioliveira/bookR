...
Para extrair a tabela da ANOVA pode também obtida, usando a função `get_anova_table()`, do pacote `rstatix`, com o `mod.anova2` como argumento:

```{r}
rstatix::get_anova_table(mod.anova2)
```

Como existe uma interação estatisticamente significativa (*F*(2, 22) = 30,4, *P* < 0,0001) entre tratamento e o tempo, deve-se prestar atenção a este fato. Os efeitos principais isolados são secundários. 
...
1. Uma ANOVA de medidas repetidas de dois fatores foi realizada para avaliar o 
efeito de diferentes tratamentos dietéticos ao longo do tempo no escore de autoestima.

2. Houve uma interação estatisticamente significativa entre `tratamento` e `tempo` no escore de autoestima, *F*(2, 22) = 30,4, p < 0,0001. Por isso, o efeito da variável `tratamento` foi analisado em cada ponto de tempo. Os valores de *P* foram ajustados usando o método de correção de testes múltiplos de Bonferroni. O efeito da variável `tratamento` foi significativo em t2 (*P* = 0,036) e t3 (*P* = 0,00051), mas não no ponto de tempo t1 (*P* = 1).
