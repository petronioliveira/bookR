...
Use a função `str()` para visualizar o conjunto de dados:^[Observe, na saída, que na variável `utiNeo` aparece palavras com acentuação (“não”). Às vezes, ao abrir o arquivo com a função `read.csv2()`, pode acontecer de esta palavra aparecer, por exemplo, como: “n\xe3o”. Louco, não é? Se ocorrer isso, use, após o nome do arquivo e separado por vírgula, o argumento `fileEncoding = “latin1”`. Dessa forma, o erro será corrigido.]  
```{r}
str(neonatos)
```
...
dn
```
### Juntando componenetes de datas
