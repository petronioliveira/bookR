...
A **Razão de Probabilidades** (*likelihood ratio*) é uma forma alternativa de descrever o desempenho de um teste diagnóstico. Alguns autores a denominam de razão de verossimilhança ^[Verossimilhança no sentido de a qualidade de algo que parece verdadeiro ou provável, que não contraria a verdade].  
...
Os resultados mostram que o NND^[Em inglês, NNH (number needed to harm).] é igual a 526. Ou seja, para evitar um IAM há necessidade de tratar 110 pacientes e a cada 526 tratados espera-se um caso de AVC, havendo um benefício bem maior quando comparado ao risco de AVC.
...
Inicialmente, será feita uma regressão logística do tipo entrada forçada (`enter`), método padrão de conduzir uma regressão que consiste em simplesmente colocar todos os preditores no modelo de regressão em um bloco e estimar parâmetros para cada um [@field2012logit].  O dataframe `dados` será usado no modelo com todos os preditores dentro da função. O objeto criado será denominado de `mod1`.
