...
e o arquivo `dadosMater15.xlsx`, amostra de 15 recém-nascidos, que pode ser obtido [**aqui**](https://github.com/petronioliveira/Arquivos/blob/main/dadosMater15.xlsx) e baixado para o seu diretório de trabalho. Esta amostra é proveniente do banco de dados original (`dadosMater.xlsx`), veja seção \@ref(sec-mater).
...
Antes de trabalahar com os dados, há necessidade de instalar e carregar alguns pacotes. O pacote `pacman()` @tyler2018pacman será utilizado pela sua versatilidade em buscar e carregar múltiplos pacotes. Será utilizada a função `p_load()`, deste pacote:
...
Vamos construir um `ggplot` padrão (Figura \@ref(fig:padrao)) que será recebido por um objeto `g`, usando `data = dados` e a estética (`aes`) usará, no eixo *x*, a variável `compRN` e, no eixo *y*, a variável `pesoRN`.
...
A este gráfico básico "vazio" adiciona-se uma camada que especifique o tipo de gemometria desejada (Figura \@ref(fig:layer1)). Será utilizada a `geom_point()` que retorna um gráfico de dispersão.
...
Para a construção de um histograma, usaremos a variável `pesoRN` e o `geom_histogram()`. Aqui, há necessidade apenas do eixo *x*, pois existe uma única variável onde se observa a sua distribuição (Figura \@ref(fig:gghist)):
...
Sera construído um gráfico de barras (Figura \@ref(fig:ggbar)) da variável `idadeCateg`, usando o `geom_bar()`.
...
Com o `geom_boxplot()`, serão construídos boxplots (Figura \@ref(fig:ggbxp)) comparando os pesos dos neonatos por sexo..
...
Para obter os dados clique [**aqui**](https://github.com/petronioliveira/Arquivos/blob/main/dadosObitos.xlsx) e baixar para o seu diretório de trabalho o arquivo `dadosObitos.xlsx`. Este conjunto de dados é constituído pelos óbitos por COVID-19 no Rio Grande do Sul, 2020-2022.  
...
Para a construção do gráfico de linha (Figura \@ref{fig:ggline)), será usado o `geom_line()`.
...
Inicialmente, serão realizadas modificações no gráfico de dispersão, construído acima com o `geom_point`, modificando a cor, de acordo com o sexo do recém-nascido (Figura \@ref(fig:point)):
...
```{r point2, warning =FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Gráfico de dispersão com cores e tamanhos diferentes dos pontos, de acordo com o sexo", fig.pos="H"}
ggplot(data = dados) +
  geom_point(aes(x = compRN, y = pesoRN, 
                 color = sexo,
                 size = sexo))
...
O formato (`shape`) pode ser colocado fora do `aes` (Figura \@ref(fig:point6)) e  [**aqui**](http://www.sthda.com/english/wiki/ggplot2-point-shapes) se encontram vários formatos que podem ser usados no `ggplot2`També é possível observar os diferentes formatos (`shape`), usando o seguinte comando, encontrado no pacote `ggpubr`:  
...
Faz-se isso, usando a função `filter()` do pacote `dplyr`. Será construído um gráficoigual ao anterior, filtrando apenas o sexo masculino (Figura \@ref(fig:point8)):
...
Usando o pacote `ggpubr` consegue-se criar um gráfico semelhante (Figura \@ref(fig:erro1)) ao anterior (média $\pm$ se), mas com o intervalo de confiança (média $\pm$ $1.96 \times se$).
...
Será usado aqui o gráfico já construído acima (Figura \@ref(fig:point7)) com pequenas alteraçõese vamos armazená-lo em um objeto, denominado `gd` (gráfico de dipersão). Isto facilita a repetição do gráfico em outros códigos, pois basta escrever `gd` e executar (Figura \@ref(fig:gd)).
...
Mostrou-se que é possível interfir na expansão da margem com o argumento `expand = TRUE` ou `FALSE` que pode ser usado também com a função `scale_y_continuous()`. Agora, será visto um exemplo de um gráfico de barras das faixas etárias das gestantes (Figura \@ref(fig:axis)):
