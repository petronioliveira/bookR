...
\chi^{2}=1,333 + 0,333 + 2,0  +0,50 =4,17
$$
...
Este valor é comparado com o $\chi^{2}_{calculado}$ para um nível de significância de 5%. Se ele é maior, rejeita-se se a $H_{0}$; caso contrário, não se rejeita. Para obter o valor *P*, pode-se usar a função `pchisq()`, onde, como argumento, coloca-se o valor do $\chi^{2}_{calculado}$, os graus de liberdade e acrescenta-se `lower.tail = FALSE` para obter a probabilidade da cauda superior, uma vez que a distribuição do quiquadrado é positiva.

Na Tabela \@ref(tab:tab2x2), o valor crítico é igual a 3,84 e, no exemplo, o $\chi^{2}_{calculado}$ é igual a 4,17, logo a valor *P* é igual a:

```{r}
pchisq (4.17, 1, lower.tail = FALSE)
```

Dessa forma, conclui-se ,com uma confiança de 95%, que os homens têm uma proporção maior de acidentes comparados às mulheres ($\chi^{2} (1) = 4,17;P=0,041$). Observe na Figura \@ref(fig:chi384) que o $\chi^{2}_{calculado}$ localiza-se a direita da linha vertical, na área vermelha de rejeição da $H_{0}$.
...
O exemplo da Tabela \@ref(tab:tab2x2), usado para mostrar a lógica do qui-quadrado, é um teste de independência ou associação. Ali, foi mostrado que existe uma associação estatisticamente significativa (P < 0,05) entre acidentes automobilísticos e o sexo masculino.
...
Os dados serão provenientes do banco de dados `dadosMater.xlsx`. bastante usado neste livro ^[Ver Seção \@ref(sec-mater)] .  Como ele já deve estar no seu diretório, fazer a leitura da seguinte maneira:
