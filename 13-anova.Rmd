...
Na Seção \@ref(sec-homovar), foram comparadas duas variâncias, usando um teste, denominado de *teste F*. Este teste, é uma razão entre duas variâncias e recebeu este nome em homenagem a Sir Ronald Aylmer *Fisher* (veja a Seção \@ref(sec-historia). A variância é uma medida de dispersão que mensura como os dados estão espalhados em torno da média (veja Seção \@ref(sec-variancia)). Quanto maior o seu valor, maior a dispersão. 
...
Dessa forma, como a distribuição *F* assume que a hipótese nula é verdadeira, é possível colocar o resultado de qualquer valor *F*, resultante do teste de ANOVA, e determinar quão consistente ele é com a hipótese nula e calcular a probabilidade. A probabilidade que se quer calcular é a probabilidade de observar uma estatística *F* que é pelo menos tão alta quanto o valor que o estudo obteve. Essa probabilidade permite determinar quão comum ou raro é o valor *F*, sob a suposição de que a hipótese nula é verdadeira. Se a probabilidade for pequena o suficiente, pode-se concluir que dados são inconsistentes com a hipótese nula. Como já foi mostrado em outros momentos, essa probabilidade é o valor *P* (Seção \@ref(sec-valorp)).   
...
Para representar, graficamente, esse resultado, foi construido o gráfico da Figura \@ref(fig:distf1020) com a função `shadeDist()` do pacote `fastGraph` (@garren1919fastgraph) , verifica-se que a área sob a curva abaixo de 0,97 é igual a 50%. Consulte a ajuda do *RStudio* para maiores detalhes dos argumentos da função.
...
A ANOVA de um fator compara o efeito de uma variável preditora (variável independente, fator) sobre uma variável contínua (desfecho). Por exemplo, verificar se a intensidade do tabagismo na gestação (não fumantes, fumantes leves, moderados ou pesados) afetam o peso dos recém-nascidos Figura \@ref(fig:bxpfumo).  
...
Estes dados estão no arquivo `dadosFumo.xlsx`. Para baixar o banco de dados, clique [**aqui**](https://github.com/petronioliveira/Arquivos/blob/main/dadosFumo.xlsx). Salve o mesmo no seu diretório de trabalho.  
...
ggplot2::ggplot(dados, aes(x=fumo, y=pesoRN)) +
  ...
  labs(x = "Tabagismo", 
       y = "Peso ao nascer (g)",
       caption = "Ponto vermelho na caixa = Média de cada grupo") +
...
abline (v = Fc, lwd = 1, lty = 2, col = "red")
text(3.2, 0.1, "5%", cex = 2, col = "red")
text(3.2, 0.27, "Região de", cex = 1.5, col = "red")
text(3.2, 0.2, "Rejeição", cex = 1.5, col = "red")
text(0.8, 0.2, "95%", cex = 2, col = "steelblue3")
```
...
Os dados estão estruturados com um desenho onde as células tem um formato 2 x 3 (Tabela \@ref(tab:tab2x3)) com  os fatores `sexo` e `alcool` e 8 indivíduos em cada célula. O fator `sexo` tem dois níveis (feminino e masculino) e o fator `alcool` tem três níveis (nenhum, 3 latas e 6 latas). Observe que o desenho é *balanceado*, pois todas as células têm o mesmo número de indivíduos. Esta estrutura é o caso mais simples; desenhos não balanceados são mais complexos.

```{r tab2x3, echo=FALSE, warning=FALSE, message=FALSE}
tab <- with(data = dados, table(sexo, alcool))
kable(tab, 
      "html",
      caption = "Distribuição Balanceada do Sexo e Consumo de Álcool") %>%
  kable_styling(full_width = F,
                latex_options = "hold_position") %>%
  kableExtra::kable_classic(html_font = "Cambria") %>% 
...
Uma opção, é apresentar os resultados como um gráfico de linhas (Figura \@ref(fig:result2)), já  mostrado anteriormente , usando a função `ggline()` do pacote `ggpubr`, com as cores do periódico *Lancet*:  
