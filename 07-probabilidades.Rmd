...
Neste caso, 10 (50%) desses lançamentos deram cara. Agora, suponha que foram feitos registros do número de caras ($n_1$) dos primeiros lançamentos (*N*) e calculadas as proporções de caras ($n_1⁄N$) todas as vezes. O resultado está na Tabela \@ref(tab:propmoeda).
...
```{r propmoeda, echo = FALSE, out.height = '80%', out.width = '80%', fig.align = 'center', fig.cap="Proporção em 20 lançamentos de moeda", fig.pos="H"}
 knitr::include_graphics("https://i.imgur.com/GpF0Y7J.png")
```
...
### Aplicando a visão frequentista no dia a dia
...
A definição frequentista também pode ser aplicada a uma medida contínua como a altura de mulheres. No conjunto de dados `dadosMater.xlsx` (veja seção \@ref(sec-mater)), encontra-se o registro da altura de 1368 mulheres. Essas alturas serão selecionadas e colocadas em um objeto `am` (**a**ltura das **m**ulheres):

```{r}
library(readxl)
library(dplyr)
am <- read_excel("Arquivos/dadosMater.xlsx") %>% 
  select(altura)
summary (am)
```
...
Logo, o interesse está na probabilidade de a variável aleatória assumir valores entre certos limites. A probabilidade de encontrar um valor exatamente de 1,60 m é quase igual a zero. 

Como se verá adiante, isto pode ser facilmente calculado no R:

```{r}
# Distância do valor de 1,60 e a média em desvios padrão (escore Z) 
z <- (1.60 - mean(am$altura))/sd(am$altura)
z

# Probabilidade de encontrar uma mulher, na população am, com 1,60 m 
prob <- pnorm (z, mean(am$altura),sd(am$altura))
prob
```

Viram? É muito próximo de zero! Se o valor da probabilidade for arredondado para 125 digitos após a vírgula, retorna zero! 
...
Desta forma, pode-se verificar quantos desvios padrão uma mulher que mede 1,725m, pertencente a esta população, está afastada da média de `r round(mean(resumo$media),2)`m. Assim:
...
Com esta função pode-se responder a pergunta feita anteriormente em relação a probabilidade de encontrar uma mulher com mais de 1,725m, equivalente a  `r round(z, 2)` desvios padrão acima da média, em uma população com média = `r resumo$media` e desvio padrão = `r resumo$dp`. 
...
A função `normal_area()` é uma função criada para desenhar uma curva normal com a área da probabilidade desejada colorida. Ela pode ser obtida [**aqui**](https://github.com/petronioliveira/Arquivos/blob/main/normal_area.R) e baixada no seu diretório. Foi usada a função `text()` para escrever o valor da probabilidade.

**Função `qnorm()`**

A função `qnorm()` permite encontrar o quantil *q* para qualquer probabilidade *p*. Portanto, a função `qnorm` é o inverso da função `pnorm()`. A sintaxe da `qnorm()` é a seguinte:
...
Agora, a função `rnorm()`será usada para gerar três vetores diferentes de números aleatórios de uma distribuição normal. 
...
media <-  mean(rnt$pesoRN, na.rm =TRUE)
media


# Desvio padrão dos pesos dos recém-nascidos a termo
dp  <-  sd(rnt$pesoRN, na.rm =TRUE)
dp
```
