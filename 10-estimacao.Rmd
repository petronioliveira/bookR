...
Serão carregados os dados do arquivo `dadosMater.xlsx`, já usados em outras seções. Para maiores detalhes deste conjunto de dados, consulte a Seção \@ref(sec-mater):

```{r}
mater <- read_excel("./Arquivos/dadosMater.xlsx")
```

### Exploração dos dados  {#sec-dadosaltura}

Será novamente considerado, para fins didáticos, como se fosse uma ‘população’.  Dessa população serão filtrados os recém-nascidos a termo (idade gestacional $\ge 37$ sem e <42 sem) que constituirá  um novo conjunto que será atribuído ao objeto `rnt`:

```{r}
rnt <- mater %>% filter(ig>=37 & ig<42)
```

Também será selecionada, a partir da convencionada ‘população’, `mater`, a variável `altura` para constituir-se no objeto `dadosAltura`:

```{r}
dadosAltura <- mater %>% select(altura)
```
...
A partir do conjunto de dados `rnt`, construído na Seção anterior, serão calculados a média e o desvio padrão da variável `pesoRN` (peso dos recém-nascidos em g) que, para fins didáticos, serão considerados os parâmetros dessa 'população':

```{r}
 mu <- round(mean(rnt$pesoRN, na.rm = TRUE))
 sigma <- round(sd(rnt$pesoRN, na.rm = TRUE))
 print(x = c(mu, sigma))
```
A seguir, será extraída, dessa população, uma amostra de *n* = 30 ^[É importante lembrar que toda vez que for extraída uma nova amostra de tamanho *n* = 30, o resultado será um conjunto de números diferentes e, em consequência, a média será diferente. Por isso, se for importante repetir o mesmo resultado, deve-se usar a função `set.seed()`. Consulte a Seção \@ref(sec-rnorm)] e calculado os mesmas medidas resumidoras, que se constituirão nas estimativas da `amostra` :

```{r}
set.seed (1234)
amostra <- rnt %>% select(pesoRN) %>% 
slice_sample(n = 30)
...
O valor de `r x_barra`g é a média amostral, $\bar{x}$, usada como um estimativa da $\mu$, é denominada de *estimativa pontual*. Como já mencionado anteriormente, espera-se que cada amostra selecionada produza um valor diferente da estatística amostral.
...
```{r twotailed, echo = FALSE, out.width = '70%', fig.align = 'center'}
...
Usando a média da amostra dos recém-nascidos de *n* = 30, $\bar{x}$= `r x_barra` g, e o desvio padrão populacional conhecido, $\sigma$= `r sigma` g, tem-se que o intervalo de confiança de 95% (IC95%), para o peso dos recém-nascidos a termo na ‘população’ de onde esta amostra é proveniente: 
...
```{r IC9599, echo = FALSE, out.width = '70%', fig.align = 'center'}
...
A `amostra1` de *n* = 30,  $\overline x$ = `r x_barra1`m e $s$ = `r s1`m. Essas estimativas servirão para o cálculo do intervalo de confiança, usando uma distribuição t bicaudal e um nível de significância $\alpha = 0,05$.
