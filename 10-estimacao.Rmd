...
```{r intconf, echo = FALSE, out.width = '80%', fig.align = 'center', fig.cap="Intervalo de Confiança.", fig.pos="H"}
...
O conjunto de dados resultante contém a altura de 1368 mulheres, que para fins didáticos será considerada a população de estudo. Será extraída uma amostra ^[Lembrar que se for retirada outra amostra da mesma população de *n* = 30, a média e o desvio padrão serão diferentes!] de 30 mulheres e fazer inferência para essa população, *supondo que os parâmetros dessa população sejam desconhecidos*.
...
Tem-se uma amostra de *n* = 30,  $\overline x = `r media`$ e $s = `r dp`$. Serão necessários os outros dados para obter o intervalo de confiança, usando uma distribuição *t* bicaudal e um nível de significância $\alpha = 0.05$:
...
Ou seja, temos uma confiança 95% de que a média populacional desconhecida se encontra entre `r ic`m
