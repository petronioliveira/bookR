...
O desvio padrão é grande em relação à média, com um coeficiente de variação de `r round(sd(mater$renda, na.rm = TRUE)/mean (mater$renda, na.rm = TRUE)*100, 2)
`% e uma mediana < média. Estas métricas junto com os coeficientes de assimetria e curtose apontam para a assimetria positiva da variável `renda familiar`. O gráfico da Figura \@ref(fig:skewpos) confirma esta afirmação:  
