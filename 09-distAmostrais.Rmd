...
A função `slice_sample()` do pacote `dplyr`extrairá uma amostra ^[Para que a cada nova amostragem retorne o mesmo conjunto de dados, é usado a função `set.seed()`(revise a Seção \@ref(sec-rnorm))] de n = 30 da população e calculada a média e o desvio padrão: 
...
Se este processo for repetido várias vezes, a cada amostra aleatória ^[Observe que se modificou o número da "semente"], serão gerados médias e desvios padrão diferentes.
...
O desvio padrão é grande em relação à média, com um coeficiente de variação de `r round(resumo$dp.sm/resumo$media.sm, 3)*100`% e uma mediana < média. Estas métricas junto com os coeficientes de assimetria e curtose apontam para a assimetria positiva da variável `renda`. O gráfico da Figura \@ref(fig:skewpos) confirma esta afirmação:  
...
Os valores da média e do desvio padrão calculados para a distribuição de probabilidade dessa população fornecem os valores dos parâmetros populacionais  $\mu$ e $\sigma$. Esses valores são $\mu$ =`r round(resumo$media.sm, 2)`*sm* ^[Salários mínimos] e $\sigma$ =`r round(resumo$dp.sm, 2)`*sm*.  
